...
class MarketplaceServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($request->hasfile('image')) {
                ...
                $key = Str::random(40);
                $image_file = $user->id . "-" . $key . "-" . time() . '.' . $file->extension();
                $file->storeAs($cpath, $image_file, 'public');
            }
    ...
    {
        $singleData = MarketplaceService::where('id', $id)->first();
        return view('backend.pages.main.admin.edit_service_view', compact('singleData'));
    }
