...
if (($trip->trip_location_from != NULL || $trip->trip_location_from != '') && ($trip->trip_location_to != NULL || $trip->trip_location_to != '')) {
	if (($trip->trip_hotel_name != NULL || $trip->trip_hotel_name != '') || ($trip->trip_rental_agency != NULL || $trip->trip_rental_agency != '') || (($trip->trip_location_to != NULL || $trip->trip_location_to != '') && ($trip->trip_location_from != NULL || $trip->trip_location_from != ''))) {
		...
		if ($trip->trip_transport == "vehicle") {
			...
			<tr nobr="true" width="100%">
			<th colspan="2">Destination : <span class="pad-left item_details">' . $trip->trip_location_from . '</span></th>
		    </tr>

			<tr style="background-color: rgb(185, 187, 189); color: rgb(0, 0, 0);">
			<td><font size="10">Starting date</font> : <span class="pad-left item_details">' . $dep_date . '</span></td>
			<td><font size="10">Starting time</font> : <span class="pad-left item_details">' . $dep_time . '</span></td>
			</tr>

			<tr style="background-color: rgb(185, 187, 189); color: rgb(0, 0, 0);">
			<td><font size="10">Ending Date</font> : <span class="pad-left item_details">' . $return_dep_date . '</span></td>
			<td><font size="10">Ending time</font> : <span class="pad-left item_details">' . $return_dep_time . '</span></td>
			</tr>
...
if (!empty($trip->trip_location_from_latlng_drivingportion) && !empty($trip->trip_location_to_latlng_drivingportion)) {
	...
	$route_map = "https://api.mapbox.com/styles/v1/mapbox/streets-v11/static/geojson(%7B%22type%22:%22FeatureCollection%22,%22features%22:[%7B%22type%22:%22Feature%22,%22geometry%22:%7B%22type%22:%22LineString%22,%22coordinates%22:" . $full_route . "%7D,%22properties%22:%7B%22stroke%22:%22%23007ACC%22,%22stroke-opacity%22:1,%22stroke-width%22:5%7D%7D]%7D),pin-s-a+007acc($origin_lon,$origin_lat),pin-s-b+007acc($destination_lon,$destination_lat)/auto/720x940@2x?access_token=$mapBoxKey";
	$pdf->Image($top_bar_image, 0, 0, 0, 0, '', '', '', false, 300, '', false, false, 0);
	$pdf->Image($b_left, 3, 275, 23, 22, '', '', '', false, 300, '', false, false, 0);
	$pdf->Image($b_left_text, 28, 287, 50, 2.5, '', '', '', false, 300, '', false, false, 0);
	$pdf->Image($b_right, 174, 275, 37, 23, '', '', '', false, 300, '', false, false, 0);
