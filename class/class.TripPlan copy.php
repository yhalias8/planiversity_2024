...
//        $elist = implode(',', $embasylist);
        ...
        if (!empty($from) && !empty($to)) {/* if (!empty($embasylist))
          $query = "INSERT INTO ".$this->table." (id_user, transport, location_from, location_to, location_from_latlng, location_to_latlng, option_weather, option_hotels, option_police, option_hospitals, option_gas, option_subway, option_embassis, embassis_list) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            $query = "INSERT INTO " . $this->table . " (id_user, transport, location_triptype, location_datel,location_datel_deptime, location_datel_arrtime ,dep_flight_no, dep_seat_no, location_dater, location_dater_deptime, location_dater_arrtime,ret_flight_no, ret_seat_no,hotel_name,hotel_date_checkin,hotel_date_checkout,rental_agency, rental_date_pick, rental_date_drop, location_from, location_to, location_from_latlng, location_to_latlng, location_from_flightportion, location_to_flightportion, location_from_latlng_flightportion, location_to_latlng_flightportion, location_from_drivingportion, location_to_drivingportion, location_from_latlng_drivingportion, location_to_latlng_drivingportion, location_from_trainportion, location_to_trainportion, location_from_latlng_trainportion, location_to_latlng_trainportion, option_weather, option_hotels, option_police, option_hospitals, option_gas, option_subway, option_taxi, option_airfields, option_busmap, option_university, option_atm, option_museum, option_church, option_embassis, option_metro, option_playground, option_subway_station, option_library, option_pharmacy, location_waypoint, location_waypoint_latlng,hotel_address,rental_agency_address,location_multi_waypoint,location_multi_waypoint_latlng, via_waypoints) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)";
//               echo $query;die('--');
            $stmt = $dbh->prepare($query);
...
//         die;
    ...
    function getStaticGmapURLForDirectionV2($origin, $destination, $key, $transport, $size = "640x300", $location_multi_waypoint_latlng = NULL, $wayptDraggable = NULL) {
        ...
        for($i = 0;$i<=count($location_multi_waypoint_latlng);$i ++) {
            // echo $i. "--";
            // $item['location_multi_waypoint_latlng']
        ...
        if ($transport == 'plane') {
            $markers = implode($markers, '&');
            // return "https://maps.googleapis.com/maps/api/staticmap?size=$size&maptype=roadmap&path=geodesic:true|weight:3|color:0xF1A033|$flight_str&$markers&scale=4&key=" . $key;
            ...
            // $path.='enc%3A' . urlencode($routes['overview_polyline']['points']);
            $markers = implode($markers, '&');
...
//            $tmp .= '';
            ...
            for ($i = 0; $i < 20; $i++) { 
                // if ($mode) {
                    $apiurl = "https://maps.googleapis.com/maps/api/distancematrix/json?units=" . $userdata['scale'] . "&mode=" . $mode . "&origins=" . urlencode($goal_address) . "&destinations=" . urlencode($googleDirection['results'][$i]['vicinity']) . "&key=" . $key;
                    // echo $apiurl;
                    $api = $this->curlRequest($apiurl);
                ...
                // } else { //$apiurl = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=".urlencode($this->trip_location_from)."&destinations=".urlencode($googleDirection['results'][$i]['vicinity'])."&key=".$key;
                    // $distance = $this->getDistance($this->trip_location_to_drivingportion, $googleDirection['results'][$i]['vicinity'], $scale, $key); // changed when check pdf issue 0km
                    // $distance = $this->getDistance($goal_address, $googleDirection['results'][$i]['vicinity'], $scale, $key);
                // }
                // echo $distance;
...
//                                  <tr>
//                                    <td width="10%"></td>
//                                    <td width="10%" align="center" valign="middle"><img src="' . (defined('CURL_PATH')?CURL_PATH:SITE) . 'images/map-icons/' . $filter . '.png" /></td>
...
//                                </table>';
                    ...
                    $tmp .= ' <tr>
                        ...
                        <td style="width: 20%;padding: 8px;line-height: 1.42857143;vertical-align: top;border-top: 1px solid #ddd;">' . $goal_address . '</td>
                    </tr>';
                }
            }
        }
...
//                                     <td width="10%" rowspan="3"><img src="' . (defined('CURL_PATH')?CURL_PATH:SITE) . 'images/map-icons/embassy.png" /></td>
...
//        </div>
    ...
    background-position: center; "></div>
            ...
            <div style="position: absolute;overflow: visible;left: -120;right: 0;width: 100mm;margin-left: auto;margin-right: auto;top:500;">
                <table style="width:110%" cellpadding="15">
                        <tr><td style="width:20%"><img src="' . (defined('CURL_PATH')?CURL_PATH:SITE) . 'images/pdf-icon/5.png" width="100px" height="100px" /></td>
    ...
    function getAccuWeatherLocation() {
        ...
        $json = $this->curlRequest("http://dataservice.accuweather.com/locations/v1/search?q=" . $search . "&apikey=gFpAEASB1wb6HF5qfhsyA5mClpOhIBIb");
    ...
    function getAccuWeatherCurrent($location, $sunrise, $sunset, $pdf = 0) {
        global $userdata;
        $data = '';
        $json = $this->curlRequest("http://dataservice.accuweather.com/currentconditions/v1/" . $location[0] . "?apikey=gFpAEASB1wb6HF5qfhsyA5mClpOhIBIb&details=true");
...
//                                        <table width="100%" cellpadding="0" cellspacing="0" border="0" align="center">
//                                          <tr style="text-align:left">
//                                             <td width="20%"><img src="' . (defined('CURL_PATH')?CURL_PATH:SITE) . 'images/weather-icons/acc/' . $imgname . '"  style="width:70px;" /></td>
//                                             <td width="80%"><span style="font-size:2.25em; color:#1C72B4;">' . $temp_cur . '</span></td>
//                                          </tr>
//                                        </table> 
//                                        <span class="date4"><span style="color:#999;">Current:</span> ' . $clouds . '</span><br />
...
//                                        <table width="100%" cellpadding="0" cellspacing="0" border="0" class="date2" >
//                                          <tr valign="middle">
//                                             <td valign="middle" width="40%"><img src="' . (defined('CURL_PATH')?CURL_PATH:SITE) . 'images/weather-icons/acc/' . $imgname . '" /></td>
//                                             <td width="60%">' . $temp_cur . '</td>
//                                          </tr>
//                                        </table>  
//                                        <span class="date4"><b>Current:</b> ' . $clouds . '</span><br />
...
//            }
    ...
    function getAccuWeatherFilters($pdf = 0) {
        ...
        if ($locationdata) {
            $json = $this->curlRequest("http://dataservice.accuweather.com/forecasts/v1/daily/5day/" . $locationdata[0] . "?apikey=gFpAEASB1wb6HF5qfhsyA5mClpOhIBIb&details=true&metric=" . $metric);
