...
function savedoctrip(doc,trip){
    setTimeout(function() {  
      ...
      $.post(SITE+"ajaxfiles/add_documents.php", {dt : doc, tp : trip},    
      function(data)
         { if(data['error'])
...
<div class="content">
    ...
    <nav class="links2">
        <ul>
            <li><a href="<?php echo SITE; ?>trip/create-timeline/<?php echo $_GET['idtrip']; ?>"><img src="<?php echo SITE; ?>images/steps-icons/step_calendar.png"></a><span class="sep1"></span></li>
            <li><a href="<?php echo SITE; ?>trip/plan-notes/<?php echo $_GET['idtrip']; ?>"><img src="<?php echo SITE; ?>images/steps-icons/step_notes.png"></a><span class="sep1"></span></li>
            <li><a href="<?php echo SITE; ?>trip/filters/<?php echo $_GET['idtrip']; ?>"><img src="<?php echo SITE; ?>images/steps-icons/step_filters.png"></a><span class="sep1"></span></li>
            <li><a class="active"><img src="<?php echo SITE; ?>images/steps-icons/step_documents.png"></a><span></span></li>
            <li><a href="<?php echo SITE; ?>trip/add-employee-profile/<?php echo $_GET['idtrip']; ?>"><img src="<?php echo SITE; ?>images/steps-icons/step_pdf_act.png"></a></li>
        </ul>
    </nav>

    <section class="main_cont no_bg">
        ...
        <div class="cont_in">
            ...
            <div id="win_details5" class="cont_blue">
            
            <div class="cont_blue_in">
            
                 <a onClick="show_win(5)" class="close1"></a><br clear="all" />
                 <div class="cont_blue_l">
                     <div class="style_form1">
                   
                     <h1>Add Your Travel Documents</h1>
                     <div class="error_style"><?php echo $output; ?></div>
                        ...
                        <div class="form_up"><label>Passport:</label><div id="fileuploader_passport">Upload</div> 
                        <div id="fileload_passport"></div><br clear="all" />
                        <input name="documents_loadpassport" id="documents_loadpassport" type="button" class="button bt_grey bt_grey2" value="REVIEW PASSPORTS ATTACHMENTS"></div>
                        <div class="form_up">
                        <label>Driver's License:</label><div id="fileuploader_driver">Upload</div> 
                        <div id="fileload_driver"></div><br clear="all" />
                        <input name="documents_loaddriver" id="documents_loaddriver" type="button" class="button bt_grey bt_grey2" value="REVIEW DRIVER'S LICENSE ATTACHMENTS"></div>
                        <div class="form_up">
                        <label>Hotel Itinerary:</label><div id="fileuploader_hotel">Upload</div> 
                        <div id="fileload_hitinerary"></div><br clear="all" />
                        <input name="documents_loadhitinerary" id="documents_loadhitinerary" type="button" class="button bt_grey bt_grey2" value="REVIEW HOTEL ITINERARY ATTACHMENTS"></div>
                        <div class="form_up">
                        <label>Travel Itinerary:</label><div id="fileuploader_flight">Upload</div>
                        <div id="fileload_fitinerary"></div><br clear="all" />
                        <input name="documents_loadfitinerary" id="documents_loadfitinerary" type="button" class="button bt_grey bt_grey2" value="REVIEW FLIGHT ITINERARY ATTACHMENTS"></div>
                        <div class="form_up">
                        <label>Additional Docs:</label><div id="fileuploader_additional">Upload</div>
                        <div id="fileload_additional"></div><br clear="all" />
                        <input name="documents_additional" id="documents_additional" type="button" class="button bt_grey bt_grey2" value="REVIEW ADDITIONAL DOCUMENTS"></div>
...
$(document).ready(function() {
 ...
 $("#fileuploader_passport").uploadFile({
  ...
  deleteCallback: function (data, pd) {
   for (var i = 0; i < data.length; i++) {
    $.post(SITE+"ajaxfiles/del_documents.php", {op: "delete",name: data[i]},
 ...
 $('#documents_loadpassport').click(function(event){ // add a passport document to DB  
    ...
    $.post(SITE+"ajaxfiles/saved_documents.php", {type : 'passport', tp : '<?php echo $id_trip ?>'},
    function(data)
       { if(data['error'])
 ...
 $('#documents_loaddriver').click(function(event){ // add a driver document to DB   
    ...
    $.post(SITE+"ajaxfiles/saved_documents.php", {type : 'driver', tp : '<?php echo $id_trip ?>'},
    function(data)
       { if(data['error'])
 ...
 $('#documents_loadhitinerary').click(function(event){ // add a hotel itinerary document to DB   
    ...
    $.post(SITE+"ajaxfiles/saved_documents.php", {type : 'hitinerary', tp : '<?php echo $id_trip ?>'},
    function(data)
       { if(data['error'])
 ...
 $('#documents_loadfitinerary').click(function(event){ // add a flight itinerary document to DB   
    ...
    $.post(SITE+"ajaxfiles/saved_documents.php", {type : 'fitinerary', tp : '<?php echo $id_trip ?>'},
    function(data)
       { if(data['error'])
 ...
 $('#documents_additional').click(function(event){ // add a flight itinerary document to DB   
    ...
    $.post(SITE+"ajaxfiles/saved_documents.php", {type : 'additional', tp : '<?php echo $id_trip ?>'},
    function(data)
       { if(data['error'])
...
function DrawPlaneDirectionsHandler(map,marker_a,marker_b) {
   ...
   var flightPlanCoordinates = [
      {lat: <?php echo $lat_from_plane; ?>, lng: <?php echo $lng_from_plane; ?>},         
      {lat: <?php echo $lat_to_plane; ?>, lng: <?php echo $lng_to_plane; ?>}
    ];
...
function AutocompleteDirectionsHandler(map,transport,lat_from,lng_from,lat_to,lng_to,marker_a,marker_b) {
  ...
  var directionsDisplay = new google.maps.DirectionsRenderer({ polylineOptions: { strokeColor: "#F08A0D" } });
        ...
        if (status == google.maps.DirectionsStatus.OK) {
            var line = new google.maps.Polyline({
             ...
             //directionsDisplay.setDirections(response);    
                ...
                infowindow.setContent("<div style='float:left; padding: 3px;'><img width='40' src='https://www.planiversity.com/images/car_icon.png'></div><div style='float:right; padding: 3px;'>"+response.routes[0].legs[0].distance.text + "<br>" + response.routes[0].legs[0].duration.text + "</div>");
    ...
    if(transport=='train'){
      ...
      directionsService.route(request, function(response, status) {
            if (status == google.maps.DirectionsStatus.OK) {
                ...
                infowindow2.setContent("<div style='float:left; padding: 3px;'><img width='40' src='https://www.planiversity.com/images/train_icon2.png'></div><div style='float:right; padding: 3px;'>"+response.routes[0].legs[0].distance.text + "<br>" + response.routes[0].legs[0].duration.text + "</div>");
