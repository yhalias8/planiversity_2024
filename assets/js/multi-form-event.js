;(function ($) {
	$.fn.multiStepForm = function (args) {
		...
		form.navigateTo = function (i) {
			...
			tabs.removeClass('current').eq(i).addClass('current').delay(500).fadeIn()
		...
		form.find('.submit').on('click', function (e) {
			...
			if (
				...
				let attachment = $('#location_document').prop('files')?.[0] ?? ''

				if ($('#document_option_yes').is(':checked') && attachment) {
					formData.append('location_document', attachment)
				}

				$.ajax({
					url: SITE + 'process/itinerary_event_process.php',
					type: 'POST',
					data: formData,
					contentType: false,
							...
							function (isConfirm) {
								...
								if (isConfirm) {
									window.location.replace(
										SITE + 'trip/connect/' + response.data
									)
								} else {
									startover()
								}
