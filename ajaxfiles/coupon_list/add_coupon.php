...
if (isset($_POST['title']) && isset($_POST['start_date']) && isset($_POST['coupon_code']) && isset($_POST['end_date']) && isset($_POST['percent']) && isset($_POST['status']) && isset($_POST['access']) && isset($_POST['auth_level'])) {
    ...
    $query = "INSERT INTO coupon (title, coupon_code, percent,start_date,end_date,status,lifetime,target_auth_level,target_plan_level,bulk_coupon,coupon_prefix,coupon_postfix) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bindValue(9, $plan_level, PDO::PARAM_STR);
    $stmt->bindValue(10, $bulk, PDO::PARAM_INT);
    $stmt->bindValue(11, $prefix, PDO::PARAM_STR);
    $stmt->bindValue(12, $postfix, PDO::PARAM_STR);
    $tmp = $stmt->execute();
    if (!$tmp) {
        $error = 'error_fail';
    }
