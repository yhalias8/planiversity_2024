...
if ($_POST['notes_text'] && $_POST['notes_idtrip']) {
        ...
        $note->put_data($id, $notes_text);
        
        if ($trip_generated == 1) {

        $data = [
            "UserId" => $trip_u_id,
            "NotificationTitle" => "New Note Added",
            "NotificationBody" => notificationBodyProcess("add", "note", $trip_title)
        ];
