...
if ($userdata['sync_googlecalendar']) {
	...
	$event['start_time'] = str_replace('-', '', $startdate);
...
$pdf->Image($logo_img, 75, 70, 65, 60, '', '', '', false, 300, '', false, false, 0);
...
$section = new top_bar('Your Itinerary', 'The travel plan of tomorrow done right today');
$html = $section->html_content();

if ($trip->trip_hotel_address != NULL || $trip->trip_hotel_address != '') {
	$hotel .= '<table class="body">
			<tr>
				<td width="10%">
...
if (count(json_decode($trip->location_multi_waypoint)) != 0 && ($trip->location_multi_waypoint != NULL && $trip->location_multi_waypoint != '[""]')) {
	...
	$multi_waypoint_seat = json_decode($trip->multi_waypoint_seat_no);
	$multi_waypoint_flight = json_decode($trip->multi_waypoint_flight_no);
		...
		$multi_waypoint .= '
				<tr>
					...
					<td width="90%" rowspan="1">
						...
						<table width="100%">
							<tr class="title">
								<td width="70%" style="color:#67758D; font-size:14px"><b>Destination:</b> <span>' . $location_multi_waypoint[$i] . '</span></td>
